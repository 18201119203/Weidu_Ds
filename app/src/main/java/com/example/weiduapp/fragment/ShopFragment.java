...
public class ShopFragment extends BasemvpFragment<ProductContract.IProductModel,ProductContract.ProductPresentervoid> implements ProductContract.IProductView,NotifyNum {
    ...
    @OnClick(R.id.jie)
    public void setJies(View view){

        List<ShopCartBean.ResultBean> list1 = new ArrayList<>();
        for (ShopCartBean.ResultBean resultBean : shopCartBean.result) {
            if (resultBean.ischelick){
                list1.add(resultBean);
            }
        }
        EventBus.getDefault().postSticky(list1);
    ...
    @Override
    public void successHomeData(HomeItemShopBean shopBean) {
    ...
    public void SuccessData(Object data) {
        ...
        if ("1001".equals(shopCartBean.status)){
            SharedPreferences sp = SpUtils.getInstance().getSp();
            SharedPreferences.Editor edit = sp.edit();
            edit.putBoolean("che_zd",false).commit();
            Toast.makeText(getActivity(),shopCartBean.message+"",Toast.LENGTH_LONG).show();
